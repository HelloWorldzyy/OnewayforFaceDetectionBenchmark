...
void SampleBBox(const Sampler& sampler, NormalizedBBox* sampled_bbox,const int height,const int width) {
  // Get random scale.
  ...
  {
  	bbox_height=scale;
  	bbox_width=scale*width/height;
  }
  else
  {
  	bbox_width=scale;
  	bbox_height=scale*height/width;
...
void GenerateSamples(const NormalizedBBox& source_bbox,
                     ...
                     vector<NormalizedBBox>* sampled_bboxes,const int height,const int width) {
  int found = 0;
  for (int i = 0; i < batch_sampler.max_trials(); ++i) {
    ...
    SampleBBox(batch_sampler.sampler(), &sampled_bbox,height,width);
